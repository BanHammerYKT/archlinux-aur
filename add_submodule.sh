...
help() {
  ...
  echo "    Syntax: $0 [Package GIT URL] [Submodule name]"
  echo ""
}

## check number of arguments
[[ $# -ne 1 && $# -ne 2 ]] && help && echo "One submodule is needed as argument !" && exit 10

! [[ $1 =~ https?://.*\.git ]] && help && echo "The URL is not a Git repo" && exit 11

## set argument as named variable
GIT_URL="$1"
MODULE=${2:-$(echo $GIT_URL | sed 's#.*/\(.*\).git#\1#')}
...
git submodule add --force "$GIT_URL" "$MODULE"
